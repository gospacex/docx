...
func TestGetOrCreateConcurrentOnlyBuildsOnce(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		...
		go func(i int) {
			defer wg.Done()
			results[i], errs[i] = getOrCreate(ctx, key, kindBucket, fn)
		}(i)
	}
	wg.Wait()
...
func TestFailureCooldownReusesErrorWithinCooldown(t *testing.T) {
	const key = "test:bucket:cooldown"
	evict(key)
	defer evict(key)
	...
	if _, err := getOrCreate(context.Background(), key, kindBucket, fn); !errors.Is(err, wantErr) {
